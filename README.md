...
プロンプト先頭に `(.venv)` が表示されていれば venv が有効です。表示されない場合は以下のいずれかの問題:
- venv が作成されていない → 上記の作成コマンドを再実行
- 活性化スクリプトが実行されていない → `.\.venv\Scripts\Activate.ps1` を再実行
- venv が Python 3.12 で作成された → `.venv` を削除して `py -3.11` で再作成
...
`build_index.py` 実行時に発生する場合は、`.env` ファイルに値が入っていないことが原因です。`.env.example` をコピーしただけでは値が空のままなので、Foundry から取得した実際のキー・エンドポイント・デプロイ名を `notepad .env` で入力してください。

確認:

```powershell
type .env
# AZURE_OPENAI_API_KEY=... のように値が入っているか確認
```

### Python 3.12 以上を使用している (3.11 をインストールしたい)

M365 Agents SDK の公式サポートは Python 3.9 ~ 3.11 です。3.12 以上では予期せぬエラーが発生する可能性があります。

**Python 3.11 を追加インストール (3.12 と共存可能):**

```powershell
winget install Python.Python.3.11
```

インストール後、新しい PowerShell ウィンドウを開き、`py -3.11` で 3.11 を明示的に指定して venv を作り直してください。
